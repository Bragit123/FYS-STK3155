...
for i in range(deg_num):
    X = FeatureMatrix(x, y, degs[i]) # Compute feature matrix
...
plt.figure()
plt.title(f"Mean square error for ordinary least squares.", fontsize=20)
...
plt.savefig("../Figures/MSEOLS.pdf", bbox_inches='tight')

plt.figure()
plt.title(f"R2-values for ordinary least squares.", fontsize=20)
...
plt.xticks(fontsize=20)
plt.legend(fontsize=20)
plt.savefig("../Figures/R2OLS.pdf", bbox_inches='tight')
