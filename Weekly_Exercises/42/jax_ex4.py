...
def GD_ada(X, y, n_iter=100):
    ...
    for i in range(n_iter):
        ...
        Giter = Giter + gradient**2
        beta = beta - eta/(delta + jnp.sqrt(Giter)) * gradient
    
    return beta

def GD_mom_ada(X, y, gamma, n_iter=100):
    n, p = jnp.shape(X)
    ...
    for iter in range(n_iter):
        ...
        Giter = Giter + gradient**2
        eta_ada = eta/(delta + jnp.sqrt(Giter))
        v = eta_ada*gradient + gamma*v
        beta = beta - v
...
def SGD_ada(X, y, n_epochs, M):
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(m):
            ...
            Giter = Giter + gradient**2
            eta_ada = eta/(delta + jnp.sqrt(Giter))
            beta = beta - eta_ada*gradient

            eta = learning_schedule(epoch*m + i)

    return beta

def SGD_mom_ada(X, y, n_epochs, M, gamma):
    n, p = jnp.shape(X) # p = degree + 1
    m = int(n/M) # M = batch_size ; m = number of minibatches
...
n_iter = 1000
beta_gd = GD_ada(X, y, n_iter)

gamma = 0.1
beta_mom = GD_mom_ada(X, y, gamma, n_iter)

M = 5
n_epochs = 50
beta_sgd = SGD_ada(X, y, n_epochs, M)

beta_sgd_mom = SGD_mom_ada(X, y, n_epochs, M, gamma)

print("beta_difference (with adagrad)")
