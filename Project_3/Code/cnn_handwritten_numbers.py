...
x_train = x_train[:][:][0:int(0.1*len(x_train[:][:]))] #The data contains 60000 samples, 6000 should be more than enough
...
y_train = to_categorical(y_train) #Transforming the labels from a single digit to an array of length 10 with the digit corresponding to the index
...
heatmap(train_accuracy, xticks=lmbd_vals, yticks=eta_vals, title="Training Accuracy", xlabel="$\lambda$", ylabel="$\eta$", filename="../Figures/cnn_train_acc.pdf")

heatmap(test_accuracy, xticks=lmbd_vals, yticks=eta_vals, title="Test Accuracy", xlabel="$\lambda$", ylabel="$\eta$", filename="../Figures/cnn_test_acc.pdf")
