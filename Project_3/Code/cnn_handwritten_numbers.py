...
activations = ["relu", "leaky_relu", "sigmoid", "linear"]
activation_names = ["RELU", "LRELU", "sigmoid", "identity"]
for k in range(len(activations)):
    for i, eta in enumerate(eta_vals):
        for j, lmbd in enumerate(lmbd_vals):
            CNN = create_convolutional_neural_network_keras(input_shape, receptive_field,
                                                  n_filters, n_hidden_neurons, n_categories,
                                                  eta, lmbd, activations[k])
    ...
    heatmap(train_accuracy, xticks=lmbd_vals, yticks=eta_vals, title=f"Training Accuracy, {activation_names[k]}", xlabel="$\lambda$", ylabel="$\eta$", filename=f"../Figures/cnn_train_acc_{activations[k]}.pdf")
    heatmap(test_accuracy, xticks=lmbd_vals, yticks=eta_vals, title=f"Test Accuracy, {activation_names[k]}", xlabel="$\lambda$", ylabel="$\eta$", filename=f"../Figures/cnn_test_acc_{activations[k]}.pdf")
